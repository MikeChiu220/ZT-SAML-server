...
// Modify Note:
// --- Ver: 1.01 ---
// Mike[2021/11/11] Debug iOS development can't receive notify
// =========================================================================================================================
...
function send_mobile_notification_request($env, $device_type, $token, $notify_type, $notify_msg)
{
	...
	syslog (LOG_INFO, "send_mobile_notification_request: $device_type, $token, $notify_type, $notify_msg");	// for test
    if ($device_type == 'iOSd') {		// Mike[2021/11/11]
		$device_type = 'iOS';
		$env = 'DEVELOPMENT';
	}
    if ($device_type == 'iOS') {
        ...
        if ($notify_type == 'Alt' || $notify_type == 'Call') {
...
//		echo 'Will send to: ' . "$url/3/device/$token with $payload";		// for test
        ...
        curl_setopt_array($http2ch, array(
          ...
          CURLOPT_HTTPHEADER => array(
            "apns-topic: {$bundleid}",
			"apns-push-type: $pushType",		// Mike[2021/01/07] Mike[2021/02/17] Del, will cause the notify cn't be received Mike[2021/09/17] Recover for test speed
		...
		syslog (LOG_NOTICE, "Send notify to: $apns_url/3/device/$token with $payload -> result: $result");		// for test
...
//					echo "result: success<br>\n";					// for test
			...
			syslog (LOG_NOTICE, "Send notify to: https://fcm.googleapis.com/fcm/send $token -> result: $result");		// for test
			curl_close( $ch );
		}
		else
