 ...
 * Modify Note:
 * --- Ver: 1.10 ---
 * Mike[2022/07/12] For NCD Scan & Lan capture can work for 2 LAN Interfaces
 * ========================================================================================================================
 */
 ...
 */
	 ...
	 */
	function _getDeviceInfo($location) {
		$curl = curl_init();
		...
		curl_close($curl);
		return (!empty($ret->device))?_object_to_array($ret->device):null;
	}
	 ...
	 */
	function _sendResponse($devices, $json) {
	 ...
	 */
	function _search($st, $timeout, $mx = null) {
		...
		// ---- [ Mike[2022/07/12] Get LAN interface name ==
		$command = '/sbin/ifconfig -s';
		exec($command,$output,$return_var);
		$WAN= strtok($output[1], ' ');
		$LAN1= strtok($output[2], ' ');
		if (strcmp($LAN1,'lo')==0) {
			$LAN1='';
			$dev_eth1=0;
		}
			...
			$dev_eth1=1;
		$ethInterface = array( $WAN , $LAN1 );
		
		for ( $index= 0; $index < sizeof($ethInterface); $index++) {
		// ---- ]*/
			$socket = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
			socket_set_option($socket, SOL_SOCKET, SO_RCVTIMEO, array('sec'=>$timeout, 'usec'=>0));
			if ($index)																			// Mike[2022/07/12]
				socket_set_option($socket, SOL_SOCKET, SO_BINDTODEVICE, $ethInterface[$index]);	// /
	 ...
	 */
	function getAllDevices($json = false, $timeout = 2, $mx = null) {
		$devices = _search('ssdp:all', $timeout, $mx);
		return _sendResponse(_sortByIP($devices), $json);
	}
	 ...
	 */
	function getAllRootDevices($json = false, $timeout = 2, $mx = null) {
		// Performs SSDP upnp:rootdevice
		$devices = _search('upnp:rootdevice', $timeout, $mx);
		//Cleaning list
		$list = [];
		foreach($devices as $device) {
			if( $device['ST'] == 'upnp:rootdevice' && empty($list[$device['UUID']]) ) {
				...
				$list[$device['UUID']]['DESCRIPTION'] = _getDeviceInfo($device['LOCATION']);
			}
		}		
		return _sendResponse(_sortByIP($list), $json);
	}
	
	/**
	 * Callable static method to fetch all VOIP devices which are responding as root devices.
	 *  
	 * It's especially useful to have a clean list of all main devices without redundant responses and services. It can cut the results count by ten.
	 *  
	 * @param	boolean		$json		Triggers a JSON output
	 ...
	 * @return	array		An array sorted by IP with the devices list and full description for each device
	 */
	function getAllVoipDevices($json = false, $timeout = 2, $mx = null) {
		// Performs SSDP upnp:rootdevice
		$devices = _search('upnp:Voip-Device', $timeout, $mx);
		//Cleaning list
		$list = [];
		if ($devices != NULL && sizeof($devices))
		foreach($devices as $device) {
			if( $device['ST'] == 'upnp:Voip-Device' && empty($list[$device['UUID']]) ) {
				$list[$device['UUID']] = $device;
				//Fetching additionnal informations
//				$list[$device['UUID']]['DESCRIPTION'] = _getDeviceInfo($device['LOCATION']);
			}
	 ...
	 */
	function getDevicesByURN($urn, $json = false, $timeout = 1, $mx = null) {
		$devices = _search($urn, $timeout, $mx);
		//Fetching additional informations
		foreach($devices as $key => $device) $devices[$key]['DESCRIPTION'] = _getDeviceInfo($device['LOCATION']);
		return _sendResponse(_sortByIP($devices), $json);
	}
	 ...
	 */
	function getDeviceByUUID($uuid, $json = false, $timeout = 1, $mx = null) {
		$device = null;
		$devices = _search('uuid:'.$uuid, $timeout, $mx);
		if(!empty($devices)) $device = $devices[0];
		//Fetching additionnal informations
		$device['DESCRIPTION'] = _getDeviceInfo($device['LOCATION']);		
		return _sendResponse($device, $json);
	}
//}


function udpSend($buf, $host, $port) {
	$socket = socket_create(AF_INET, SOCK_DGRAM, SOL_UDP);
...
function nd6RebootReq($RebootId)
{
	$host="239.255.255.250";
	$port=1900;
	$strHeader = "M-SEARCH * HTTP/1.1\r\n";
	$strHeader .= "$host:$port\r\n";
	$strHeader .= "ST:uuid:REBOOT-$RebootId\r\n";
	$strHeader .= "Man:\"ssdp:discover\"\r\n";
	$strHeader .= "MX:3\r\n\r\n";
	udpSend($strHeader, $host, $port);
}
